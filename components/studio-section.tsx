                ...
                <div className="relative w-80 h-80 flex items-center justify-center overflow-hidden">
                  <div className="absolute inset-0 z-0">
                    {[...Array(8)].map((_, i) => (
                      <div
                        key={`lx-${i}`}
                        className="absolute w-full h-[1px] bg-black dark:bg-white opacity-10 loom-x"
                        style={{
                          top: `${15 + i * 10}%`,
                    ...
                    {[...Array(8)].map((_, i) => (
                      <div
                        key={`ly-${i}`}
                        className="absolute h-full w-[1px] bg-black dark:bg-white opacity-10 loom-y"
                        style={{
                          left: `${15 + i * 10}%`,
                  ...
                  <div className="relative z-10 w-48 h-64 border border-black/10 dark:border-white/10 rounded-2xl flex items-center justify-center">
                    ...
                    <Cpu
                      size={48}
                      className="text-black dark:text-white animate-pulse opacity-40"
                    />
                    <div className="absolute inset-0 p-8 grid grid-cols-4 grid-rows-6 gap-2">
                      {[...Array(24)].map((_, i) => (
                        <div
                          key={i}
                          className="bg-black dark:bg-white rounded-[1px] transition-all duration-1000"
                  ...
                  <div className="absolute bottom-6 text-center w-full z-20">
                    <div className="flex justify-center gap-1 mb-3">
                      {[...Array(3)].map((_, i) => (
                        <div
                          key={i}
                          className="w-1 h-1 bg-black dark:bg-white rounded-full animate-bounce"
                          style={{ animationDelay: `${i * 0.2}s` }}
                        />
                      ))}
                    </div>
                    <p className="text-[9px] font-bold uppercase tracking-[0.5em] opacity-60">
                      Neural Fabric Synthesis...
                    </p>
                  </div>
                </div>
              </div>
